    ...
    <div class="container-fluid px-3 px-lg-4 py-4">
        ...
        <form id="trendFilterForm" method="GET" action="{{ route('trends.index') }}" class="card bg-dark border-secondary border-opacity-25 shadow-sm mb-4">
            <div class="card-body row g-3 align-items-end">
                ...
                <div class="col-md-5"><label for="port" class="form-label small text-white-50">Lokasi referensi tren cuaca</label><select id="port" name="port" class="form-select bg-black text-white border-secondary"><option value="">{{ $ports->isEmpty() ? 'Koordinat negara (Open-Meteo)' : 'Pilih otomatis (data terbaru)' }}</option>@foreach($ports as $port)<option value="{{ $port->id }}" @selected($selectedPort?->id === $port->id)>{{ $port->name }}</option>@endforeach</select></div>
                <div class="col-md-2"><button class="btn btn-primary w-100" type="submit"><i class="bi bi-arrow-repeat me-1"></i>Tampilkan</button></div>
            </div>
        </form>

        @php($stormTone = $weatherStormStatus === 'High' ? 'danger' : ($weatherStormStatus === 'Medium' ? 'warning' : ($weatherStormStatus === 'Low' ? 'success' : 'secondary')))
        <div class="alert alert-info bg-info bg-opacity-10 border-info border-opacity-25 text-info small mb-4"><i class="bi bi-info-circle me-1"></i>Weather Trend memakai <strong>{{ $weatherSource }}</strong>. Tersedia <strong>{{ $weatherObservationCount }} titik</strong>{{ $weatherLatestAt ? ', pembaruan terbaru '.$weatherLatestAt : '' }}. <span class="badge text-bg-{{ $stormTone }} ms-1">{{ strtoupper($weatherStormStatus ?? 'N/A') }} STORM{{ $weatherRiskScore !== null ? ' · '.$weatherRiskScore.'/100' : '' }}</span> Nilai snapshot ini diteruskan ke komponen cuaca pada Risk Trend. Negara tanpa port menggunakan koordinat referensi negara dari Open-Meteo.</div>
